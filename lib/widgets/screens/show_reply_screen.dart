...
class ShowReplyScreenArguments {
  ShowReplyScreenArguments({@required this.post});

  final Post post;
}

class ShowReplyScreen extends StatelessWidget {
  const ShowReplyScreen({@required this.args});

  static const route = '/show_reply';
  final ShowReplyScreenArguments args;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ChangeNotifierProvider(
        create: (context) => ShowReplyScreenStateNotifier(
          postRepository: context.read<FirebasePostRepository>(),
          sourcePost: args.post,
        ),
        child: Consumer<ShowReplyScreenStateNotifier>(
          builder: (context, notifier, _) {
            final state = notifier?.state ?? ShowReplyScreenLoading();

            if (state is ShowReplyScreenLoadFailure) {
              return Center(
                child: Container(
            ...
            if (state is ShowReplyScreenLoadSuccess) {
